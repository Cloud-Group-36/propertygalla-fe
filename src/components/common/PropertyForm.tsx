...
export default function PropertyForm({ mode, initialData, onSuccess, onCancel }: PropertyFormProps) {
  ...
  return (
    <Box bg="white" _dark={{ bg: "gray.800" }} p={5} borderRadius="md" shadow="md" mb={6}>
      <Text fontWeight="bold" mb={4}>
        {mode === "add" ? "Add New Property" : "Edit Property"}
      </Text>

      <VStack gap={4} align="stretch">
        <Field name="title" label="Title" value={formData.title} onChange={handleChange} />
        <Field name="description" label="Description" type="textarea" value={formData.description} onChange={handleChange} />
        <Field name="state" label="State" value={formData.state} onChange={handleChange} />
        <Field name="city" label="City" value={formData.city} onChange={handleChange} />
        <Field name="neighborhood" label="Neighborhood" value={formData.neighborhood} onChange={handleChange} />
        <Field name="location" label="Location" value={`${formData.city}, ${formData.state}, ${formData.neighborhood}`} onChange={() => {}} disabled />
        <Field name="rooms" label="Rooms" type="number" value={formData.rooms} onChange={handleChange} />
        <Field name="bathrooms" label="Bathrooms" type="number" value={formData.bathrooms} onChange={handleChange} />
        <Field name="parking" label="Parking Spaces" type="number" value={formData.parking} onChange={handleChange} />
        <Field name="area" label="Area (sq ft)" type="number" value={formData.area} onChange={handleChange} />
        <Field name="price" label="Price (RM)" type="number" value={formData.price} onChange={handleChange} />
        <Field name="images" label="Upload Images" type="file" onChange={(e) => handleImageChange(e as React.ChangeEvent<HTMLInputElement>)} accept="image/*" multiple />
